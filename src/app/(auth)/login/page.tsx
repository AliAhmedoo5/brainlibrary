...
import { Mail, Lock, Sparkles, BookOpen, Loader2, CheckCircle2, AlertCircle, Eye, EyeOff, CloudOff } from 'lucide-react';
      ...
      <div className="glass-panel w-full max-w-md rounded-3xl p-8 shadow-2xl border border-[var(--border-color)] bg-[var(--bg-surface)] text-[var(--text-primary)] transition-all duration-300 relative z-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)]">{dict.welcomeBack}</p>
          </div>
        </div>
        ...
        {!isFirebaseConfigured && (
          <div className="mb-5 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300 flex items-start gap-3 animate-fadeIn">
            ...
            <div>
              ...
              <p className="text-gray-400 leading-normal">Cloud sync (Email/Google login) is disabled because Firebase credentials are not configured in `.env.local`. You can start writing instantly in offline-first Demo Mode.</p>
            </div>
          </div>
        )}
